...
<head>
  ...
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet"
    ...
    <script type="text/javascript">
		function origen(str){ //funcion para crear la conexion asincronica
			var conexion;
			...
			conexion.open("GET", "idaD.php?o="+str, true);
			//po ultimo enviamos la conexion
			conexion.send();

		}
    function destino(str){ //funcion para crear la conexion asincronica
			var conexion;
			...
			//abrimos una conexion asincronica usando el metodo GET y le enviamos la variable c
			conexion.open("GET", "lugarT.php?d="+str, true);
			//po ultimo enviamos la conexion
			conexion.send();
      ...
      <div class="sec1 shadow-lg p-3 mb-5 bg-body rounded">
        <section class="formulario">
          ...
          <form  action="CRUD/crear.php" method="post" name="form-work">
            ...
            <select name="selectt" id="select" class="form-select" onclick="origen(this.value)">
            ...
            <input type="date">
            <h5>Vuelo de ida disponible</h5>
            <div id="div">
              <select name="ida" id="idaV" class="col-4 form-control">
                <option selected>Primero selecciona origen y destino de vuelo</option>
                  <?php
                  // include "lugarV.php";
                   // include("conexion.php");
                    //$consulta = "SELECT * FROM idaV";
                    //$ejecutar = sqlsrv_query ($con, $consulta);
                    //while($fila=sqlsrv_fetch_array($ejecutar)){
                    //$time=$fila['horaV'];
                    //echo "<option value=".$fila['ID_idaV'].">".$time->format('H:i')."</option>";
                    //} 
                  ?>
              </select>
            </div>
            <h5>Vuelo de regreso disponible</h5>
            <select>
              <option selected>Seleccione el origen de salida</option>
                <?php
                // include "lugarV.php";
